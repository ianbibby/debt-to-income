...
(function() {
  ...
  DTICalculator = (function() {
    function DTICalculator() {
      this.creditScoreThreshold = __bind(this.creditScoreThreshold, this);
      this.creditScoreDeductions = __bind(this.creditScoreDeductions, this);
      this.matrixLookup = __bind(this.matrixLookup, this);
      this.calculate = __bind(this.calculate, this);
      this.qualifying = __bind(this.qualifying, this);
      this.maxPayment = __bind(this.maxPayment, this);
      this.maxDti = __bind(this.maxDti, this);
      this.creditScore = __bind(this.creditScore, this);
      this.ratio = __bind(this.ratio, this);
      this.sumFormGroup = __bind(this.sumFormGroup, this);
      $.getJSON('/loan-matrix.json', (function(_this) {
        return function(data) {
          return _this.matrix = data;
        };
      })(this));
      $(".monthly-income input, .debt-payments input, .credit-score input").focusout((function(_this) {
        return function() {
    ...
    DTICalculator.prototype.sumFormGroup = function(group_name) {
      var el, inputs, values;
      ...
      values = (function() {
        ...
        for (_i = 0, _len = inputs.length; _i < _len; _i++) {
          el = inputs[_i];
          _results.push(parseFloat(el.value));
        }
        return _results;
      })();
      return values.reduce(function(acc, e) {
        if (isNaN(e)) {
          return acc;
        }
        return acc + e;
    ...
    DTICalculator.prototype.ratio = function(debt, monthly) {
      var n;
      n = debt / monthly;
      if (isNaN(n)) {
        return 0;
    ...
    DTICalculator.prototype.creditScore = function() {
      return parseInt($("#input-credit-score").val());
    };

    DTICalculator.prototype.maxDti = function() {
      var cs;
      cs = this.creditScore();
      if (cs > 720) {
        return 1;
      } else if ((640 <= cs && cs <= 720)) {
        return 0.55;
      } else if ((590 <= cs && cs <= 639)) {
        return 0.45;
      } else if ((530 <= cs && cs <= 589)) {
        return 0.40;
      } else if (cs < 530) {
        return 0.35;
      } else {
    ...
    DTICalculator.prototype.maxPayment = function(ratio) {
      var n;
      n = (this.maxDti() - (ratio / 100)).toFixed(2);
      console.log("" + (this.maxDti()) + " - (" + (ratio / 100) + ") = " + n);
      if (isNaN(n)) {
        return 0;
      }
      return parseInt(n * 100);
    ...
    DTICalculator.prototype.qualifying = function(maxPaymentPercentage, totalMonthlyIncome) {
      var n;
    ...
    DTICalculator.prototype.calculate = function() {
      var actual, amt, i, lookup, maxPm, post_deductions, quali, ratio, str, tdebt, tmnth, _i, _results;
      tmnth = this.sumFormGroup('monthly-income').toFixed(2);
      tdebt = this.sumFormGroup('debt-payments').toFixed(2);
      ratio = this.ratio(tdebt, tmnth);
      maxPm = this.maxPayment(ratio);
      quali = this.qualifying(maxPm, tmnth);
      $('#monthly-income').val(tmnth);
      $('#debt-payments').val(tdebt);
      $('#dti-ratio').val(NumberHelpers.number_to_percentage(ratio, {
        precision: 0
      }));
      $('#max-payment').val(NumberHelpers.number_to_percentage(maxPm, {
        precision: 0
      }));
      $('#qualifying-amount').val(NumberHelpers.number_to_currency(quali));
      lookup = this.matrixLookup(quali);
      actual = lookup["loan_amount"] || 0;
      if (actual > 0) {
        post_deductions = actual - this.creditScoreDeductions(actual);
      ...
      for (i = _i = 1; _i <= 8; i = ++_i) {
        str = "payment-" + i;
        amt = lookup[str] || 0;
        _results.push($("." + str).html(NumberHelpers.number_to_currency(amt)));
      }
      return _results;
    };

    DTICalculator.prototype.matrixLookup = function(qualified) {
      var candidate, candidate_delta, delta, k, taac, v, _ref;
      candidate = 0;
      candidate_delta = 9999999;
      _ref = this.matrix;
      for (k in _ref) {
        v = _ref[k];
        taac = parseFloat(k).toFixed(2);
    ...
    DTICalculator.prototype.creditScoreDeductions = function(amount) {
      var delta, score, threshold;
      score = this.creditScore();
      if (isNaN(score) || score === void 0) {
        return 0;
      }
      threshold = this.creditScoreThreshold(amount);
      delta = threshold - score;
      if (delta > 0) {
        return 100 * parseInt(delta / 50);
      } else {
        return 0;
      }
    };

    DTICalculator.prototype.creditScoreThreshold = function(amount) {
      if ((100.00 <= amount && amount <= 300)) {
        return 500;
      } else if ((300.01 <= amount && amount <= 500)) {
        return 550;
      } else if ((500.01 <= amount && amount <= 800)) {
        return 650;
